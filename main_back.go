...
const (
	...
	IOMMU_GET_HW_INFO = 0x3b8a // _IO(';', 0x8a)
)
...
type iommuHwInfo struct {
    ...
    DataLen         uint32
    DataUptr        uint64
    InOutDataType   uint32 // union with out_data_type
    OutMaxPasidLog2 uint8
    Reserved        [3]uint8
    OutCapabilities uint64
}

func getSsidSize(iommuFd int, devID uint32) (int, error) {
    ...
    if errno != 0 {
        return 0, fmt.Errorf("IOMMU_GET_HW_INFO failed: %v", errno)
    }

    if info.OutMaxPasidLog2 == 0 {
        return 0, nil // no PASID/SSID support
    }
    return int(info.OutMaxPasidLog2), nil // this is exactly your "ssidsize" in bits
}
...
func adaptiveBind(devFd, iommuFd int) error {
	...
	if e1 == 0 {
		...
		ssidSize, err := getSsidSize(iommuFd, args16.OutDevid)
		if err == nil {
			fmt.Printf("  [IOMMU_GET_HW_INFO] SSID/PASID size (bits): %d\n", ssidSize)
			// Use ssidSize in your XML output / capabilities instead of the old parse value
		} else {
			// fallback to your old parser if you want
			fmt.Printf("  [IOMMU_GET_HW_INFO] (fallback needed): %v\n", err)
		}
		return nil
	}
	...
	if e1 == unix.EINVAL || e1 == unix.E2BIG || e1 == unix.ENOTTY {
		...
		if e2 == 0 {
			fmt.Printf("  [Bind] Success with mainline 24-byte extension (dev_id: %d).\n", args24.OutDevid)
                        ssidSize, err := getSsidSize(iommuFd, args24.OutDevid)
			if err == nil {
				fmt.Printf("  [IOMMU_GET_HW_INFO] SSID/PASID size (bits): %d\n", ssidSize)
				// Use ssidSize in your XML output / capabilities instead of the old parse value
			} else {
